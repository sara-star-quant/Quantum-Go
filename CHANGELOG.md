...
- Reliable rekey over the datagram channel: the CH-KEM keys exceed the MTU, so it needs a fragmented sub-handshake. Until it lands, a session is bounded to one epoch's key (`Send` refuses past the per-epoch packet budget) and there is no datagram throughput benchmark yet.
