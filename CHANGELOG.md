...
- **X-Wing KEM suite**: the standardized X-Wing KEM (ML-KEM-768 + X25519, draft-connolly-cfrg-xwing-kem) is now a registered, negotiable suite (`SuiteXWing`), wrapping cloudflare/circl's certified implementation for byte-exact interop. A client that leads with X-Wing negotiates it end to end on both transports; static-key authentication works with an X-Wing identity independent of the negotiated ephemeral suite. `quantum-tunnel keygen --suite x-wing` mints an X-Wing identity, and key/pin files are tagged with their suite (`chkem.ParseTaggedKeyPair` / `ParseTaggedPublicKey`). The default client still leads with CH-KEM-v1, so existing handshakes are unchanged.

### Changed
- **Length-prefixed KEM material on the wire (protocol 5.0, wire-incompatible)**: the ClientHello public key and static ciphertext, the ServerHello ciphertext, and the rekey payload now carry a 2-byte length prefix instead of a fixed CH-KEM-v1 size, so the wire can carry suite-dependent key and ciphertext sizes (X-Wing's smaller key share, future suites). The KEM suite validates the exact size when it parses the material. This bumps the protocol version to 5.0; both 4.0 and 5.0 are pre-release, so the double major bump is cosmetic (no tagged build ships 4.0).
- **KEM-suite negotiation on the wire (protocol 4.0, wire-incompatible)**: the ClientHello and ServerHello now carry the negotiated KEM suite id (the ClientHello also lists every suite the client supports). The server adopts the suite the client's key share uses and echoes it; the client rejects a mismatched echo, and the suite is bound into the handshake transcript, so a downgrade also fails the Finished MAC. Only `SuiteCHKEMv1` is registered, so behavior is unchanged, but the wire is now suite-aware. The added fields bump the protocol version to 4.0; `IsCompatible` is major-only, so a 3.x and a 4.0 peer reject each other at the handshake. HelloRetryRequest (for a server that lacks the client's suite) and variable-size suites (length-prefixed material) follow with X-Wing.
